(() => {
  ...
  function setNutrientCapFromWorld() {
    // Nutrient bar represents progress toward the NEXT colony expedition.
    // As the world grows, the requirement rises (but stays reasonable).
    const g = growthScore();
    const base = 140;                 // early game: quick to fill
    const perColony = 55;             // ramps with colony count
    const perGrowth = 18;             // mild ramp with economy growth
    NUTRIENT.cap = clamp(
      Math.floor(base + colonies.length * perColony + g * perGrowth),
      140,
      2400
    );
  }
  ...
  function bossNutrientMultiplier(w) {
    if (!w?.isBoss) return 1;
    switch (w.special) {
      case "FIRE_DOGE":        return 2.35;
      case "SOL_STORM":        return 2.00;
      case "ICE_QUEEN":        return 1.90;
      case "EVENT_HORIZON":    return 2.15;
      case "WYRM_EMPEROR":     return 2.60;
      default:                return 1.75;
    }
  }

  // Spend nutrients toward founding the next colony.
  // If the bar fills, we dispatch an expedition and the bar rolls over.
  function spendNutrientsTowardColony() {
    if (colonies.length >= MAX_COLONIES) return;

    // Cap might change as the world grows; use a loop so overflow still counts.
    let safety = 6; // prevent runaway loops in weird edge cases
    while (NUTRIENT.value >= NUTRIENT.cap && safety-- > 0) {
      const ok = scheduleFounderMission();
      if (!ok) return; // don't consume nutrients if we couldn't dispatch
      NUTRIENT.value = Math.max(0, NUTRIENT.value - NUTRIENT.cap);

      pushLog("mile", "Nutrient saturation reached — expedition dispatched");
      playSfx("ping", 1);
      worldShake(6, 360);
    }
  }
      ...
      for (const w of c.worms) {
        ...
        for (let i = NUTRIENT.orbs.length - 1; i >= 0; i--) {
          ...
          if (d < bite) {
            ...
            addNutrientValue(n.val * bossNutrientMultiplier(w));
  ...
  function scheduleFounderMission() {
    if (colonies.length >= MAX_COLONIES) return false;
  ...
  function step(dt, time) {
    ...
    // If the nutrient bar fills, dispatch a new colony expedition
